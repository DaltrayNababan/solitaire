...
public class Stats {
    ...
    public Stats(final SolitaireCG solitaire, final SolitaireView view) {
        ...
        TextView tv = solitaire.findViewById(R.id.text_title);
        tv.setText(String.format(Locale.getDefault(), solitaire.getString(R.string.stats_for_game), rules.GetPrettyGameTypeString()) + "\n\n");
        tv = solitaire.findViewById(R.id.text_wins);
        tv.setText(String.format(solitaire.getString(R.string.stats_wins_and_attempts), wins, attempts));
        tv = solitaire.findViewById(R.id.text_percentage);
        tv.setText(String.format(solitaire.getString(R.string.stats_winning_percentage), String.format(Locale.getDefault(), "%.2f", ratio)));
        if (bestTime != -1) {
            int seconds = (bestTime / 1000) % 60;
            int minutes = bestTime / 60000;
            tv = solitaire.findViewById(R.id.text_best_time);
            tv.setText(String.format(solitaire.getString(R.string.stats_fastest_time), String.format(Locale.getDefault(), "%d:%02d", minutes, seconds)));
        }
        if (rules.HasScore()) {
            tv = solitaire.findViewById(R.id.text_high_score);
            tv.setText(String.format(solitaire.getString(R.string.stats_high_score), highScore));
        }


        final Button accept = solitaire.findViewById(R.id.button_accept);
        accept.setOnClickListener(v -> {
            MPlayer.playClicked(solitaire);
            solitaire.CancelOptions();
        });
        final Button clear = solitaire.findViewById(R.id.button_clear);
