...
class SelectCard {
    ...
    public void InitFromAnchor(CardAnchor cardAnchor) {
        ...
        System.arraycopy(card, 0, mCard, 0, card.length);
    ...
    public Card[] DumpCards() {
        ...
        if (mValid) {
            ...
            if (GetCount() >= 0) System.arraycopy(mCard, 0, ret, 0, GetCount());
            Clear();
        }
        return ret;
    }
    ...
    public void Scroll(float dy, int minY, int maxY) {
        ...
        if ( /* Ensure at least one card */mCardCount > 0 && ( /* Scrolling up so ensure last card visible */
                                ((dy > 0.0) && ((mCard[mCardCount - 1].GetY() - dy) < minY)) ||
                                        /* Scrolling down so ensure first card visible */
                                        ((dy < 0.0) && ((mCard[0].GetY() - dy) > (maxY - Card.HEIGHT))))) {
